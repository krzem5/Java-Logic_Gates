...
if (subprocess.run(["javac","-encoding","utf8","-d","../build"]+jfl).returncode!=0):
	sys.exit(1)
os.chdir(cd)
with zipfile.ZipFile("build/logic_gates.jar","w") as zf:
...
if ("--run" in sys.argv):
	subprocess.run(["java","-Dfile.encoding=UTF8","-jar","build/logic_gates.jar"])
